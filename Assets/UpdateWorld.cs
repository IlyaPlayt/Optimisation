...
{
    [SerializeField] private MeshRenderer world;
    ...
    void Start()
    {
        ...
        worldMaterial = world.material;
        camera = Camera.main;


        texture = Instantiate(worldMaterial.mainTexture) as Texture2D;
        ...
        worldMaterial.mainTexture = texture;
        texture.Apply();
    }
    ...
    void SimulateWorld(Texture2D texture)
    {
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                var countNeighbor = CountNeighbourColor(x, y, Color.black, texture);
                ...
                if (countNeighbor > 0 && texturePixel == Color.white)
                {
                ...
                //Mining Property
                if (countNeighbor > 0 && texturePixel == Color.yellow)
                {
                ...
                if (countNeighbor == 0 &&
                    (texturePixel == Color.green ||
                     texturePixel == Color.blue ||
                     texturePixel == Color.magenta))
                {
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            {
                lastPoint = new Vector2((int) (ray.textureCoord.x * width),
                    (int) (ray.textureCoord.y * height));
                worldMaterial.mainTexture = texture;

                DrawPixelLine((int) (ray.textureCoord.x * width),
                    (int) (ray.textureCoord.y * height),
        ...
        if (Input.GetMouseButton(1))
        {
            ...
            if (Physics.Raycast(camera.ScreenPointToRay(Input.mousePosition), out ray))
            {
                lastPoint = new Vector2((int) (ray.textureCoord.x * width),
                    (int) (ray.textureCoord.y * height));
                worldMaterial.mainTexture = texture;
                texture.SetPixel((int) (ray.textureCoord.x * width),
                    (int) (ray.textureCoord.y * height), Color.white);
                texture.Apply();
            }
